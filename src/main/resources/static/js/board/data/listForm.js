...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderSearchResults(data, totalElements) {
        ...
        if (saveForm) {
            const dynamicUrl = '/masterpage_sys/board/data/saveForm'; // 동적으로 설정할 URL
        ...
        data.forEach((item, index) => {
            ...
            <!-- 제목 -->
            <td>
                <a href="/masterpage_sys/board/data/detailForm/${item.bbsIdx}" class="jv-btn underline01">${item.title || '제목 없음'}</a>
            </td>
            <!-- 작성일 -->
