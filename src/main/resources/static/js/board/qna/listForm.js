...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateSearchState() {
        ...
        searchState.currentPage = 0; // 검색 시 항상 첫 페이지로 이동
    }
    ...
    function createSearchUrl() {
        ...
        url.searchParams.set('boardType', searchState.boardType);
    ...
    function renderSearchResults(data, totalElements) {
        ...
        if (saveForm) {
            const dynamicUrl = '/masterpage_sys/board/qna/save'; // 동적으로 설정할 URL
        ...
        data.forEach((item, index) => {
            ...
            <td>
                <a href="/masterpage_sys/board/qna/${item.bbsIdx}" class="jv-btn underline01">${item.title || '제목 없음'}</a>
            </td>
            <td>${item.createdDate || '-'}</td>
            <td><a href="/masterpage_sys/board/qna/${item.bbsIdx}" class="jv-btn fill04">수정하기</a></td>
